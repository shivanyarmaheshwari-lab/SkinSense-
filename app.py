...
def voice_button(text: str, label: str | None = None) -> None:
    ...
    components.html(
        ...
        ></div>
        <script>
            function speakSkinSense{widget_id}() {{
                    ...
                    const targetLang = {json.dumps(lang)}.toLowerCase();
                    const languageRoot = targetLang.split("-")[0];
                    const preferredVoice =
                        voices.find(voice => voice.lang && voice.lang.toLowerCase() === targetLang) ||
                        voices.find(voice => voice.lang && voice.lang.toLowerCase().startsWith(languageRoot)) ||
...
<head>
  ...
  <title>SkinSense Screening Report</title>
  <style>
    body {{
      margin: 0;
      background: #edf8fc;
    ...
    .green {{ background: #d7fae8; border-color: #a9dfbb; }}
    .red {{ background: #fff0ef; border-color: #efb9b5; }}
    .yellow {{ background: #fff8bf; border-color: #efd891; }}
...
<body>
  <main class="page">
    <section class="header">
      {logo_html}
      <div>
        <h1>SkinSense Screening Report</h1>
        <div class="subtitle">AI skin screening support for fishing communities</div>
      </div>
    </section>
    ...
    <section class="grid">
      <div class="card">
        ...
        <div class="value">{escape(str(user_name))}</div>
      </div>
      <div class="card">
        <div class="label">Date issued</div>
        <div class="value">{escape(str(issued_at))}</div>
      </div>
      <div class="card">
        <div class="label">Possible condition</div>
        <div class="value">{escape(prediction.condition)}</div>
      </div>
      <div class="card">
        <div class="label">AI match accuracy</div>
        <div class="accuracy">{prediction.accuracy_percentage}%</div>
      </div>
    </section>

    <section class="section blue">
      <h2>What this means</h2>
      <div>{escape(prediction.explanation)}</div>
    </section>
    <section class="section green">
      <h2>One home-care step</h2>
      <div>{escape(prediction.what_to_do)}</div>
    </section>
    <section class="section red">
      <h2>One thing to avoid</h2>
      <div>{escape(prediction.what_to_avoid)}</div>
    </section>
    <section class="section yellow">
      <h2>When to see a dermatologist</h2>
      <div>{escape(prediction.dermatologist)}</div>
    </section>
    <section class="section blue">
      <h2>Urgent warning signs</h2>
      <div>Seek urgent medical help if there is fever, pus, severe pain, fast-spreading redness, swelling, bleeding, or trouble breathing.</div>
    </section>

    <section class="footer">
      SkinSense is an AI screening aid, not a medical diagnosis. Please consult a dermatologist or qualified health professional for confirmation and treatment.
    </section>
  </main>
</body>
</html>"""
