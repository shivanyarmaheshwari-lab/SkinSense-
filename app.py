        ...
        <script>
            ...
            function listenSkinSense{widget_id}() {{
                ...
                if (!SpeechRecognition) {{
                    output.textContent = "Speech input is not supported in this browser. Try Chrome, or use the phone keyboard microphone.";
                    return;
                }}
                ...
                recognition.onresult = function(event) {{
                    ...
                    if (command) {{
                        output.textContent = "Heard: " + heardText + ". Opening...";
                        sendSkinSenseCommand{widget_id}(command);
                    }}
                }};
                recognition.onerror = function() {{
                    output.textContent = "I could not hear that clearly. Please try again.";
                }};
            ...
            function commandLabelsSkinSense{widget_id}(command) {{
                const labels = {{
                    ...
                    back: [{json.dumps(t("back_options"))}, "Back", "Back to upload", "Options पर वापस जाएं", "Options कडे परत जा"],
                    home: [{json.dumps(t("back_options"))}, "Start over", "Home"],
                    logout: ["Logout"],
            ...
            function sendSkinSenseCommand{widget_id}(command) {{
                ...
                if (clickVisibleSkinSenseButton{widget_id}(command)) {{
                    output.textContent = "Heard command. Pressing the button...";
                    return;
                }}
                try {{
                    let baseUrl = "";
                    ...
                    output.innerHTML = 'Heard command. Opening...<br><a href="' + targetUrl + '" target="_parent" style="color:#1d6fd6;font-weight:800;">Tap here if it does not open</a>';
                    ...
                    output.textContent = "I heard you, but this browser could not control the app. Try Chrome.";
                }}
            }}
        </script>
        """,
...
def handle_voice_command() -> None:
    ...
    if command == "result":
        ...
        if st.session_state.get("image_prediction"):
            go("questions")
        st.session_state["voice_notice"] = "Please upload an image and answer the questions before opening the result."
        st.rerun()
    ...
    st.session_state["voice_notice"] = "I heard you, but I could not match that voice command."
    st.rerun()
...
<head>
  ...
  <title>SkinSense Clinical Screening Summary</title>
  <style>
    :root {{
      --ink: #10243f;
      --muted: #52657a;
...
<body>
  <main class="page">
    <header class="top">
      {logo_html}
      <div>
        <h1>SkinSense Clinical Screening Summary</h1>
        <div class="subtitle">AI-assisted skin screening summary for discussion with a dermatologist</div>
        <div class="document-note">Generated by SkinSense. This document is informational and should not be used as a confirmed diagnosis.</div>
      </div>
    </header>

    <section class="section">
      <div class="section-title">Patient and Screening Details</div>
      <table>
        <tr>
          <th>Name</th>
        ...
        <tr>
          ...
          <td class="condition">{escape(prediction.condition)}</td>
        </tr>
        <tr>
          <th>AI match accuracy</th>
    ...
    <section class="section">
      ...
      <div class="box">
        ...
        {escape(prediction.explanation)}
      </div>
    </section>

    <section class="section">
      <div class="section-title">Care Guidance</div>
      <div class="box">
        <strong>One home-care step</strong>
        {escape(prediction.what_to_do)}
      </div>
      <div class="box">
        <strong>One thing to avoid</strong>
        {escape(prediction.what_to_avoid)}
      </div>
      <div class="box">
        <strong>When to see a dermatologist</strong>
    ...
    <section class="section">
      ...
      <div class="box warning">
        ...
        Seek urgent medical help if there is fever, pus, severe pain, fast-spreading redness, swelling, bleeding, or trouble breathing.
      </div>
      <div class="box disclaimer">
        <strong>Medical disclaimer</strong>
        SkinSense is an AI screening aid, not a medical diagnosis. Please consult a dermatologist or qualified health professional for confirmation, treatment, and medication advice.
      </div>
    </section>

    <footer class="footer">
      <span>SkinSense screening summary</span>
      <span>Generated: {escape(str(issued_at))}</span>
    </footer>
  </main>
</body>
</html>"""
...
def dermatologist_page() -> None:
    ...
    if not location:
        ...
        try:
            ...
            st.warning("I could not load nearby clinic records right now. Please use the Google Maps button above.")

        st.subheader(t("top_places"))
        if not display_places:
            st.info("I could not find named skin-care clinic records for this exact area in public map data. Use the Google Maps button above for live clinic names, phone numbers, and timings.")
...
def questions_page() -> None:
    ...
    question_voice_text = " ".join(
        f"Question {index}. {question}. {t('question_voice_suffix')}"
...
def result_page() -> None:
    ...
    voice_button(
        f"{t('result_voice_intro')} {t('possible_condition')}: {prediction.condition}. "
        f"{t('ai_accuracy')}: {prediction.accuracy_percentage} percent. "
        f"{prediction.explanation}. "
        f"{t('what_can_do')}: {prediction.what_to_do}. "
        f"{t('what_avoid')}: {prediction.what_to_avoid}. "
        f"{t('when_derm')}: {prediction.dermatologist}. "
        f"{t('disclaimer')}"
    )
