...
TEXT = {
    "English": {
        ...
        "distance": "Distance",
        "no_nearby_places": "I could not find nearby public clinic or hospital records for this exact address. Try adding a road name, landmark, area, and city.",
    ...
    "हिंदी": {
        ...
        "distance": "दूरी",
        "no_nearby_places": "इस exact address के पास public clinic या hospital records नहीं मिले. Road name, landmark, area और city जोड़कर try करें.",
    ...
    "मराठी": {
        ...
        "distance": "अंतर",
        "no_nearby_places": "या exact address जवळ public clinic किंवा hospital records सापडले नाहीत. Road name, landmark, area आणि city जोडून try करा.",
    ...
    if not location:
        ...
        try:
            ...
            if geocoded is None:
                ...
                broad_map_query = care_query("government hospital dermatologist public skin clinic low cost dermatologist", location)
                components.iframe(maps_embed_url(broad_map_query), height=430, scrolling=False)
                st.link_button(t("open_maps"), maps_search_url(broad_map_query), use_container_width=True)
                ...
                if not places:
                    st.warning(t("no_nearby_places"))
                    fallback_query = care_query("government hospital dermatology skin OPD", location)
                    st.link_button(t("open_maps"), maps_search_url(fallback_query), use_container_width=True)
                else:
                    st.subheader(t("top_places"))
                    for index, place in enumerate(top_nearby_places(places), start=1):
                        address = place_address(place) or t("address_not_listed")
                        contact = place_contact(place) or t("contact_not_listed")
                        with st.container(border=True):
                            st.markdown(f"### {index}. {place['name']}")
                            st.write(f"**{t('distance')}:** {place['distance_km']:.1f} km")
                            st.write(f"**{t('place_type')}:** {place['kind'].title()}")
                            st.write(f"**{t('address')}:** {address}")
                            st.write(f"**{t('contact')}:** {contact}")
                            st.link_button(
                                t("open_maps"),
