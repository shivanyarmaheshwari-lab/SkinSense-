...
def danger_risk_score(condition: str, answers: dict[str, str]) -> tuple[int, str]:
    """Convert the predicted condition and symptom answers into an urgency-style danger score."""
    base_risk = {
    ...
    risk = base_risk.get(condition, 35)
    ...
    for question, answer in answers.items():
        ...
        if any(word in question_text for word in ("pus", "bleeding", "bad smell", "fluid", "ooze", "discharge")):
            risk += 22 * answer_weight
        elif any(word in question_text for word in ("fever", "swelling", "warm", "redness")):
            risk += 20 * answer_weight
        elif any(word in question_text for word in ("spreading", "getting bigger", "spread", "worse", "worsening")):
            risk += 16 * answer_weight
        elif any(word in question_text for word in ("pain", "hurt", "burn")):
            risk += 12 * answer_weight
        elif any(word in question_text for word in ("more than", "week", "2 weeks", "long")):
            risk += 8 * answer_weight
        elif any(word in question_text for word in ("sea water", "seawater", "wet work", "fishing")):
            risk += 5 * answer_weight
        elif "itch" in question_text:
            risk += 4 * answer_weight

    risk_percentage = int(round(max(0, min(100, risk))))

    if risk_percentage >= 75:
        urgency = "Urgent: please see a doctor or dermatologist as soon as possible."
    elif risk_percentage >= 56:
        urgency = "Medium-high: book a dermatologist visit soon, especially if it is spreading or painful."
    elif risk_percentage >= 26:
        urgency = "Moderate: care for it at home, but see a dermatologist if it does not improve."
    else:
        urgency = "Low: it may heal with simple home care, but monitor it."

    return risk_percentage, urgency
...
def combine_image_and_questions() -> Prediction:
    ...
    risk_percentage, urgency = danger_risk_score(condition, answers)
...
def result_page() -> None:
    ...
    st.markdown(
        ...
        <div class="result-panel">
            <div class="result-kicker">Danger risk percentage</div>
            <div class="risk-big">{prediction.risk_percentage}%</div>
            <p>This means how urgently the skin problem may need care, not how confident the AI is.</p>
        </div>
