...
def voice_button(text: str, label: str | None = None) -> None:
    """Render a browser text-to-speech button without adding deployment dependencies."""
    ...
    components.html(
        f"""
        <div style="display:flex; gap:8px; width:100%; margin:4px 0 14px 0;">
            <button
...
def start_page() -> None:
    ...
    voice_button(
        f"Welcome to SkinSense. {t('tagline')} Enter your name, choose your language, and press continue. {t('disclaimer')}"
    )
...
def google_maps_care_searches(location: str) -> list[dict[str, Any]]:
    """Fallback rows when public map records do not contain enough named skin-care places."""
    search_rows = [
        ("Dermatologists nearby", f"dermatologist near {location}"),
        ("Skin clinics nearby", f"skin clinic near {location}"),
        ("Government hospital skin OPD", f"government hospital dermatology OPD near {location}"),
        ("Public clinic for skin care", f"public health clinic dermatologist near {location}"),
        ("Lower-cost skin care", f"charitable low cost skin clinic near {location}"),
    ]
    return [
        {
            "name": label,
            "kind": "Google Maps search",
            "lat": None,
            "lon": None,
            "distance_km": None,
            ...
            "tags": {
                "display_name": "Open Google Maps to see the closest matching places, phone numbers, and current timings.",
            },
        }
        for label, query in search_rows
    ]


def fill_to_five_care_options(location: str, places: list[dict[str, Any]]) -> list[dict[str, Any]]:
    filled = list(places[:5])
    if len(filled) >= 5:
        return filled
    existing_names = {str(place.get("name", "")).lower() for place in filled}
    for fallback in google_maps_care_searches(location):
        if str(fallback["name"]).lower() in existing_names:
            continue
...
def place_address(place: dict[str, Any]) -> str:
    if place.get("is_search_fallback"):
        return str(place.get("tags", {}).get("display_name", "Open Google Maps for details."))
    tags = place.get("tags", {})
...
def place_contact(place: dict[str, Any]) -> str:
    if place.get("is_search_fallback"):
        return "Available in Google Maps when listed"
    tags = place.get("tags", {})
...
def care_choice_page() -> None:
    ...
    voice_button(f"{t('choice_title')} {t('choice_help')} Option one: {t('check_skin')}. {t('check_skin_help')} Option two: {t('find_dermatologist')}. {t('find_dermatologist_help')}")
    ...
    if not location:
        ...
        try:
            ...
            if geocoded is None:
                ...
                display_places = fill_to_five_care_options(location, display_places)
                used_search_fallback = any(place.get("is_search_fallback") for place in display_places)
        except Exception:
            logger.exception("Nearby care lookup failed")
            display_places = google_maps_care_searches(location)
        ...
        if used_search_fallback:
            st.info("Some rows are Google Maps searches because public clinic records near this address were limited.")
        ...
        for index, place in enumerate(display_places, start=1):
            ...
            distance = "Search Maps" if place.get("distance_km") is None else f"{float(place['distance_km']):.1f} km"
            fallback_class = " search-fallback" if place.get("is_search_fallback") else ""
            st.markdown(
                f"""
                <div class="clinic-card{fallback_class}">
...
def upload_page() -> None:
    ...
    voice_button(f"{t('upload_title')}. {t('upload_help')} Use take picture to click a new photo, or upload image to choose from your gallery.")
...
def questions_page() -> None:
    ...
    question_voice_text = " ".join(
        f"Question {index}. {question}. Answer yes, no, sometimes, or I don't know."
...
def result_page() -> None:
    ...
    voice_button(
        f"Your SkinSense result. Possible condition: {prediction.condition}. "
        f"AI match accuracy: {prediction.accuracy_percentage} percent. "
        f"{prediction.explanation}. "
        f"What you can do: {prediction.what_to_do}. "
        f"What to avoid: {prediction.what_to_avoid}. "
        f"When to see a dermatologist: {prediction.dermatologist}. "
        f"{t('disclaimer')}"
    )
